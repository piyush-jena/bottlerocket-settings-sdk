...
/// HugepageConfig contains all the knobs relevant to 1 type of Hugepage.
/// We have 2 knobs - count, which contains the requested number of hugepages for the instance or node-wise.
#[derive(Debug, Clone, PartialEq, Default, Serialize, Deserialize)]
...
/// Supported transparent hugepages knobs in Bottlerocket
#[derive(Debug, Clone, PartialEq, Default, Serialize, Deserialize)]
...
mod test_hugepages_settings {
    use super::{HugepageSize, HugepagesSettings};
    use std::convert::TryFrom;

    #[test]
    fn pool_count_table_deserializes_and_round_trips() {
        let settings: HugepagesSettings =
            serde_json::from_str(r#"{"static": {"essential": true, "1Gi": {"count": "2"}}}"#)
                .unwrap();
        ...
        assert_eq!(
            json,
            serde_json::json!({"static": {"1Gi": {"count": "2"}, "essential": true}})
        );
    }
